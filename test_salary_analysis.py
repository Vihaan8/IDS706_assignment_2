...
class TestSystemTests(unittest.TestCase):
    ...
    def test_full_analysis_pipeline(self):
        ...
        test_data = pd.DataFrame(
            {
                "Salary Estimate": ["$50K-$70K"] * 30
                + ["$70K-$90K"] * 30
                + ["$90K-$110K"] * 30
                + ["$110K-$130K"] * 30,
                "Rating": np.random.uniform(2.0, 5.0, 120),
                "Size": ["51-200 employees"] * 30
                + ["201-500 employees"] * 30
                + ["501-1000 employees"] * 30
                + ["1001-5000 employees"] * 30,
    ...
    def test_edge_cases_handling(self):
        ...
        empty_df = pd.DataFrame(
            {"Salary Estimate": [], "Rating": [], "Size": [], "Industry": []}
        )
...
class TestPerformanceTests(unittest.TestCase):
    ...
    def test_pandas_vs_polars_speed(self):
        ...
        start = time.time()
        clean_data(df_pandas.copy())
        pandas_time = time.time() - start

        # Time polars
        df_polars = pl.DataFrame(test_data_dict)
        start = time.time()
        clean_data_polars(df_polars.clone())
